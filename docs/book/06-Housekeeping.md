...
**通常，不能指望 `finalize()` ，你必须创建其他的"清理"方法，并明确地调用它们。所以看起来，`finalize()` 只对大部分程序员很难用到的一些晦涩内存清理里有用了。但是，`finalize()` 还有一个有趣的用法，它不依赖于每次都要对 `finalize()` 进行调用，这就是对象终结条件的验证。** **（WILSON：就是比如程序要求某个对象在被清理之前必须完成某些操作，成为终结条件，那么通过finalize就可以很好的发现这些终结条件没有被执行的BUG）**
